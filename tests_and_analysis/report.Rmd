...
![Consort Diagram](figures/consort_diagram.png)
...
![Question Format](figures/question_format.png)
...
plot_bars_by_gender <- function(outcome_col, passage_gender) {
  outcome_plot <- ggplot() +
    geom_bar(aes(x=outcome_col, fill=passage_gender), position=position_dodge2(preserve = "single"), alpha=0.8) + 
...
The average `warmth` rating given to the female character was `r round(test_warmth$estimate[1], 3)` and the average `warmth` rating given to the male character was `r round(test_warmth$estimate[2], 3)`. The average treatment effect with the `warmth` metric (the average difference in perceived `warmth` between female character and male character) is `r round(test_warmth$estimate[1] - test_warmth$estimate[2], 3)`. This test resulted in a p-value of `r round(test_warmth$p.value, 3)` and standard error of `r round((test_warmth$conf.int[2] - test_warmth$conf.int[1]) / 3.92, 3)`.
...
The average `competence` rating given to the female character was `r round(test_competence$estimate[1], 3)` and the average `competence` rating given to the male character was `r round(test_competence$estimate[2], 3)`. The average treatment effect with the `competence` metric (the average difference in perceived `competence` between female character and male character) is `r round(test_competence$estimate[1] - test_competence$estimate[2], 3)`. This test resulted in a p-value of `r round(test_competence$p.value, 3)` and standard error of `r round((test_competence$conf.int[2] - test_competence$conf.int[1]) / 3.92, 3)`.
...
The average `respect` rating given to the female character was `r round(test_respect$estimate[1], 3)` and the average `respect` rating given to the male character was `r round(test_respect$estimate[2], 3)`. The average treatment effect with the `respect` metric (the average difference in perceived `respect` between female character and male character) is `r round(test_respect$estimate[1] - test_respect$estimate[2], 3)`. This test resulted in a p-value of `r round(test_respect$p.value, 3)` and standard error of `r round((test_respect$conf.int[2] - test_respect$conf.int[1]) / 3.92, 3)`.
...
Before examining our regression results, we performed a covariate balance check to test for inconsistencies in our randomization scheme. To compare a null intercept-only model to a full model with the covariate information we collected, we conducted an F-test to analyze the amount of additional variance explained by the full model. With a p-value of `r round(cov_balance_check$"Pr(>F)"[2], 3)`, we failed to reject the null hypothesis. In terms of our balance check, the test does not provide strong evidence of covariate imbalance or randomization errors across the treatment groups.
...
stargazer(
  warmth_model, 
  competence_model, 
  respect_model, type="latex", 
  se = list(sqrt(diag(warmth_model$vcovHC_)), 
...
```{r, echo=FALSE}
knitr::kable(d_final[ , .(avg_warmth = round(mean(warmth), 3), avg_competence = round(mean(competence), 3), avg_respect = round(mean(respect), 3), recognized = round(mean(recognize_passage == 'Yes'), 3)), keyby=.(passage_gender, source)], format = "markdown")
```
...
The average `warmth` rating given to the female character was `r round(test_warmth_filtered$estimate[1], 3)` and the average `warmth` rating given to the male character was `r round(test_warmth_filtered$estimate[2], 3)`. The average treatment effect with the `warmth` metric (the average difference in perceived `warmth` between female character and male character) is `r round(test_warmth_filtered$estimate[1] - test_warmth_filtered$estimate[2], 3)`. This test resulted in a p-value of `r round(test_warmth_filtered$p.value, 3)` and standard error of `r round((test_warmth_filtered$conf.int[2] - test_warmth_filtered$conf.int[1]) / 3.92, 3)`.
...
The average `competence` rating given to the female character was `r round(test_competence_filtered$estimate[1], 3)` and the average `competence` rating given to the male character was `r round(test_competence_filtered$estimate[2], 3)`. The average treatment effect with the `competence` metric (the average difference in perceived `competence` between female character and male character) is `r round(test_competence_filtered$estimate[1] - test_competence_filtered$estimate[2], 3)`. This test resulted in a p-value of `r round(test_competence_filtered$p.value, 3)` and standard error of `r round((test_competence_filtered$conf.int[2] - test_competence_filtered$conf.int[1]) / 3.92, 3)`.
...
The average `respect` rating given to the female character was `r round(test_respect_filtered$estimate[1], 3)` and the average `respect` rating given to the male character was `r round(test_respect_filtered$estimate[2], 3)`. The average treatment effect with the `respect` metric (the average difference in perceived `respect` between female character and male character) is `r round(test_respect_filtered$estimate[1] - test_respect_filtered$estimate[2], 3)`. This test resulted in a p-value of `r round(test_respect_filtered$p.value, 3)` and standard error of `r round((test_respect_filtered$conf.int[2] - test_respect_filtered$conf.int[1]) / 3.92, 3)`.
...
stargazer(
  ...
  filtered_respect_model, 
  type="latex", 
  se = list(sqrt(diag(filtered_warmth_model$vcovHC_)), 
...
\begin{figure}[H]
  \includegraphics{figures/short_story_excerpt.png}
  \caption{Original excerpt from the short story "I, Robot" by Isaac Asimov}
\end{figure}

\newpage

\begin{figure}[H]
  \includegraphics{figures/female_passage_excerpt.png}
  \caption{Modified excerpt from Treatment 1: Passage with Female Character}
\end{figure}

\begin{figure}[H]
  \includegraphics{figures/male_passage_excerpt.png}
  \caption{Modified excerpt from Treatment 2: Passage with Male Character}
\end{figure}
...
intent_plot_pilot <- plot_bars_by_gender(d_pilot$well_intentioned, d_pilot$passage_gender) + ylim(0, 10)+ 
  labs(title="Well-Intent. Rating Dist. (Pilot)", x="Well-Intentioned Rating", y="Count") +
  theme(legend.position = "none")
...
(warm_plot_pilot + intent_plot_pilot)  
(competent_plot_pilot + capable_plot_pilot) 
(respect_plot_pilot + plot_spacer())
...
intent_plot_final <- plot_bars_by_gender(d_final$well_intentioned, d_final$passage_gender) + 
  labs(title="Well-Intent. Rating Dist. (Final)", x="Well-Intentioned Rating", y="Count") +
  theme(legend.position="none")
...
(warm_plot_final + intent_plot_final)
(competent_plot_final + capable_plot_final)
(respect_plot_final + plot_spacer())
```
...
(enjoy_reading_dist + enjoy_scifi_dist)
(age_dist + books_read_dist)
(education_dist + plot_spacer())
```
